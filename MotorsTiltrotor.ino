...
void MotorsTiltrotor::set_update_rate(uint16_t speed_hz)
{
	...
	hal.rcout->set_freq(get_motor_mask(), _speed_hz);
}
...
void MotorsTiltrotor::output_min()
{
	...
	// 向四个舵机，分别制定不同通道的最小值和中值
	hal.rcout->write(pgm_read_byte(&_motor_to_channel_map[MOTORS_CH_AILERON]), _rc_roll.radio_trim);
	hal.rcout->write(pgm_read_byte(&_motor_to_channel_map[MOTORS_CH_TAIL]), _rc_pitch.radio_trim);
	hal.rcout->write(pgm_read_byte(&_motor_to_channel_map[MOTORS_CH_YAW]), _rc_yaw.radio_trim);
	hal.rcout->write(pgm_read_byte(&_motor_to_channel_map[MOTORS_CH_TILTROTOR]), _rc_tiltrotor.radio_min);

	tilt = _rc_tiltrotor.radio_min;
}
...
void MotorsTiltrotor::output_armed()
{
	...
	int16_t rpy_out[MAX_NUM_MOTORS]; // buffer so we don't have to multiply coefficients multiple times.
	int16_t motor_out[MAX_NUM_MOTORS];    // final outputs sent to the motors
	...
	if (_rc_autotilt.radio_in - _rc_autotilt.radio_min > 
		(_rc_autotilt.radio_max - _rc_autotilt.radio_min)*0.8 )
		_auto_tilt = true;

	if (_rc_autotilt.radio_in - _rc_autotilt.radio_min < 
		(_rc_autotilt.radio_max - _rc_autotilt.radio_min)*0.2 )
		_auto_tilt = false;

	// 优先计算倾转因子

	if (_auto_tilt) {
		int16_t d = tilt - _rc_tiltrotor.radio_in;
		if (-TILT_STEP < d && d < TILT_STEP) tilt =  _rc_tiltrotor.radio_in;
		else if (d < 0) tilt += TILT_STEP;
		else if (d > 0) tilt -= TILT_STEP;
	}
	_tilt_factor = 1.0f - sin((tilt - _rc_tiltrotor.radio_min)*0.5f*3.1415926f / (_rc_tiltrotor.radio_max - _rc_tiltrotor.radio_min));
	...
	if (_rc_throttle.servo_out == 0) {
		...
		for (i = 0; i<MAX_NUM_MOTORS; i++) {
			if (motor_enabled[i]) {
				...
				rpy_out[i] = _rc_roll.pwm_out * _roll_factor[i] * _tilt_factor +
					_rc_pitch.pwm_out * _pitch_factor[i] * _tilt_factor;
		...
		for (i = 0; i<MAX_NUM_MOTORS; i++) {
			if (motor_enabled[i]) {
				...
				rpy_out[i] = rpy_out[i] +
					yaw_allowed * _yaw_factor[i] * _tilt_factor;
	...
	//_auto_tilt_old = _auto_tilt;

	// 判断当前倾转状态

/*	if (tilt - _rc_tiltrotor.radio_min > 
		(_rc_tiltrotor.radio_max - _rc_tiltrotor.radio_min)*0.99 )
		_status = MOTORS_STATUS_H;
	if (tilt - _rc_tiltrotor.radio_min < 
		(_rc_tiltrotor.radio_max - _rc_tiltrotor.radio_min)*0.01 )
		_status = MOTORS_STATUS_V;
*/


	// 更新倾转状态，利用通道信号的变化来控制倾转方向
/*
	if (_auto_tilt == false && _auto_tilt_old == true && _status== MOTORS_STATUS_H) {
		_status = MOTORS_STATUS_H2V;
		_time = 0;
	}

	if (_auto_tilt == true && _auto_tilt_old == false && _status== MOTORS_STATUS_V) {
		_status = MOTORS_STATUS_V2H;
		_time = 0;
	}

	switch (_status) {
		case MOTORS_STATUS_M:
		break;
		case MOTORS_STATUS_V:
			tilt = _rc_tiltrotor.radio_min;
		break;
		case MOTORS_STATUS_H:
			tilt = _rc_tiltrotor.radio_max;
		break;
		case MOTORS_STATUS_A:
			tilt = _rc_tiltrotor.radio_min + (_rc_tiltrotor.radio_max - _rc_tiltrotor.radio_min)
					* _time / TIME_MAX;
			++_time;
			if (_time == TIME_MAX)
				_status = MOTORS_STATUS_H;
		break;
		case MOTORS_STATUS_H2V:
			tilt = _rc_tiltrotor.radio_max - (_rc_tiltrotor.radio_max - _rc_tiltrotor.radio_min)
					* _time / TIME_MAX;
			++_time;
			if (_time == TIME_MAX)
				_status = MOTORS_STATUS_V;
...
*/
	...
	int16_t ray_aileron = (_rc_roll.radio_out - _rc_roll.radio_trim)*(1.0f - _tilt_factor) + _rc_roll.radio_trim;
	int16_t ray_tail = (_rc_pitch.radio_out - _rc_pitch.radio_trim)*(1.0f - _tilt_factor) + _rc_pitch.radio_trim;
	...
	hal.rcout->write(pgm_read_byte(&_motor_to_channel_map[MOTORS_CH_AILERON]), ray_aileron);
	hal.rcout->write(pgm_read_byte(&_motor_to_channel_map[MOTORS_CH_TAIL]), ray_tail);
	hal.rcout->write(pgm_read_byte(&_motor_to_channel_map[MOTORS_CH_TILTROTOR]), tilt);
	hal.rcout->write(pgm_read_byte(&_motor_to_channel_map[MOTORS_CH_YAW]), _rc_yaw.radio_out);
}
...
{
	for (int8_t i = 0; i<MAX_NUM_MOTORS; i++) {
		remove_motor(i);
	}
}
